...
const propertyRanges = ['json', 'email', 'phone', 'year', 'photo', 'check', 'url', 'model'] // year - not yet working :)
...
const booleanMetadata = _.extend({}, common)

const numberMetadata = _.extend(_.pick(propTypes, [
  'range',
  'min',
  'max',
  'minDate',
...
const stringMetadata = _.extend(_.pick(propTypes, [
  'ref',
  'range',
  'pattern',
  // 'patterns',
...
const enumMetadata = _.extend(_.pick(propTypes, [
  'oneOf'
]), common)

const dateMetadata = _.extend(_.pick(propTypes, [
  'min',
  'max',
  'minDate',
  'maxDate'
...
const objectMetadata = _.extend(_.pick(propTypes, [
  'range',
  'ref',
  'inlined',
  'allowToAdd',
...
const photoMetadata = _.extend(_.pick(propTypes, [
  'mainPhoto',
  'component',
...
const arrayMetadata = _.extend(_.pick(propTypes, [
  'range',
  'items',
  'inlined',
  'allowToAdd',
...
function validateProperty ({ model, propertyName }) {
  ...
  if (propertyName[0] === '_' &&
    ...
    if (!baseProperties[propertyName] || !_.isEqual(baseProperties[propertyName], property)) {
...
function validateObjectProperty ({ model, property }) {
  ...
  if (property.range === 'photo'  ||  property.range === 'model') {
...
function validateArrayProperty ({ model, property }) {
  ...
  if (!isLocallyDefined) {
    const inner = _.omit(items, 'backlink')
    inner.type = 'object'
