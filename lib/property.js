...
const propertyRanges = ['json', 'email', 'phone', 'year', 'photo', 'check', 'url', 'model', 'document', 'troolean', 'objects'] // year - not yet working :)
...
function validateProperty ({ model, propertyName, models }) {
  ...
  for (let attr in propertyMetadataValidators) {
    if (attr in property) {
      ...
      try {
        validate({ model, property, propertyName, models })
      } catch (err) {
        handleError(err)
      }
    }
...
function validateFileProperty ({ model, property, models }) {
  typeforce(fileMetadata, property, true)
  const ref = property.ref || property.items.ref
  if (ref   &&  ref !== 'tradle.Photo'  && ref !== 'tradle.File' && (!models[ref]  ||  models[ref].subClassOf !== 'tradle.File')) {
...
function isDocumentProperty (prop, models) {
  // HACK
  return prop.range === 'document' || prop.ref === 'tradle.File' || (models[prop.ref]  &&  models[prop.ref].subClassOf === 'tradle.File')
}

function validateObjectProperty ({ model, property, models }) {
  let metadata
  if (isPhotoProperty(property))
    metadata = photoMetadata
  else if (isDocumentProperty(property, models))
  ...
  if (property.range === 'document'  ||  property.dataBundle)
    return validateFileProperty({ model, property, models })
}
...
function validateArrayProperty ({ model, property, models }) {
