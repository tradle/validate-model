...
const rawPropTypes = {
  ...
  coverPhoto: 'Boolean',
  virtual: 'Boolean',
  sample: typeforce.oneOf(typeforce.String, typeforce.Object)
}
...
function validateProperty ({ model, propertyName }) {
  ...
  if (propertyName[0] === '_' && propertyName !== '_t') {
    if (model.id !== 'tradle.Object' && model.id !== 'tradle.Message') {
      throw new Error('properties must not begin with an underscore')
    }
  }
...
function validateArrayProperty ({ model, property }) {
  ...
  if (!isLocallyDefined) {
    const inner = omit(items, 'backlink')
    inner.type = 'object'
