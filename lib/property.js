...
const propertyRanges = ['json', 'email', 'password', 'phone', 'year', 'photo', 'check', 'url', 'model', 'document', 'troolean', 'objects', 'property', 'time'] // year - not yet working :)
...
function validateProperty ({ model, propertyName }) {
  ...
  for (const attr in propertyMetadataValidators) {
    if (attr in property) {
...
function validateFileProperty ({ model, property }) {
  ...
  for (const p in props) {
    if (!props[p].dataBundle) { continue }
    if (cnt) {
...
function validateObjectProperty ({ model, property }) {
  const metadata = isPhotoProperty(property)
    ? photoMetadata
    : (
        isDocumentProperty(property)
          ? fileMetadata
          : objectMetadata
      )
...
function validateNumberProperty ({ property }) {
  if (property.numberFormat) {
    if (typeof property.numberFormat !== 'object') {
      throw new Error(`Property ${property.name} has 'numberFormat' annotation. It must be an object like {maximumFractionDigits: [integer number]}`)
    }
    const { maximumFractionDigits } = property.numberFormat
    if (!maximumFractionDigits || typeof maximumFractionDigits !== 'number') {
      throw new Error(`Property ${property.name} has 'numberFormat' annotation. The 'maximumFractionDigits' value should be an integer number`)
    }
    if (maximumFractionDigits % 1 !== 0) {
      throw new Error(`Property ${property.name} has 'numberFormat' annotation. The 'maximumFractionDigits' value should be an integer number`)
    }
  }
...
function validateArrayProperty ({ model, property }) {
  ...
  const { ref, backlink, range } = items
  if (backlink) {
    if (!ref) {
...
function validateSet ({ model, property }) {
  if (property.type === 'array') {
    throw new Error("'set' can't be used for 'array' type properties")
  }
  let formula = property.set
  ...
  const setF = new Function(...keys, `return ${formula}`)
  try {
    setF(...values)
  } catch (err) {
...
function checkMimeTypes ({ model, propertyName }) {
  ...
  const isPhoto = ref === 'tradle.Photo'
...
;(function copyMetadataValidatorsToExports () {
  for (const p in propertyMetadataValidators) {
