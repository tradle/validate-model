...
function validateObjectProperty ({ model, propertyName }) {
  const prop = model.properties[propertyName]
  const metadata = prop.range === 'photo' || prop.ref === 'tradle.Photo' ? photoMetadata : objectMetadata
...
function checkGroup ({ model, propertyName, groupName }) {
  ...
  return find(val, prop => {
    if (!model.properties[prop]) {
      throw new Error(`property "${propertyName}" group "${groupName}" references non-existent property "${prop}"`)
    }
  })
}
