...
function validateModel (model) {
  ...
  checkGroups(model)
  if (model.verifiableAspects) {
    validateVerifiableAspects(model)
  }

  if (model.sort && !model.properties[model.sort]) {
    throw new Error(`"sort" ${referencesUnknownProp(model.sort)}`)
  }

  if (model.customerCanHaveMultiple && model.subClassOf !== 'tradle.FinancialProduct') {
