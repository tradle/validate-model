...
const metadataTypes = {
  ...
  style: maybeObject,
  primaryKeys: typeforce.maybe(function primaryKeys (val) {
    return typeof val.hashKey === 'string' &&
      (!val.rangeKey || typeof val.rangeKey === 'string')
  })
}
...
const interfaceValidators = {
  'tradle.Context': validateContextModel
}

const enumIdRegex = /^[a-zA-Z0-9]+$/
const enumItemType = typeforce.compile({
  id: val => {
    ...
    if (!enumIdRegex.test(val)) {
      throw new Error(`"id" must adhere to the following regex: ${enumIdRegex.toString()}`)
    }
...
function validateModel (model) {
  ...
  if (interfaces) {
    for (let interfaceId in interfaceValidators) {
...
function validateContextModel(model) {
  let { contextId } = model.properties
  if (!contextId  ||  contextId.type !== 'string')
    throw new Error('implementors of "tradle.Context" interface should have property "contextId" of type "string"')
}
