...
function validateFinancialProductModel (model) {
  ...
  if (!Array.isArray(model.interfaces) || model.interfaces.indexOf('tradle.ChatItem') === -1) {
    throw new Error('subclasses of "tradle.FinancialProduct" should implement interface "tradle.ChatItem"')
  }
}

function validateFormModel (model) {
  if (!Array.isArray(model.interfaces) || model.interfaces.indexOf('tradle.ChatItem') === -1) {
    throw new Error('subclasses of "tradle.Form" should implement interface "tradle.ChatItem"')
  }
}
