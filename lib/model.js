...
function _validateModel (model, models) {
  if (model.type !== MODEL) {
  ...
  try {
    validateProperties(model, models)
  } catch (err) {
...
function validateProperties (model, models) {
  const properties = model.properties
  typeforce(typeforce.Object, properties)

  for (let p in properties){
    try {
      validateProperty({ model, propertyName: p, models })
    } catch (err) {
