...
const EmployeeRolesModel = require('./employee-roles')
...
exports.EmployeeRolesModel = EmployeeRolesModel
...
exports.validateAllow = function validateAllow({model, property}) {
  let allow
  if (property)
  ...
  if (typeof allow !== 'object')
    throw new Error(`"allow" should be a JSON object`)
  let roles = EmployeeRolesModel.enum.map(elm => elm.id)
  let actions = ['edit', 'create']
  for (let p in allow) {
    if (!actions.includes(p))
      throw new Error(`"allow" refers to the unknown property ${p})}`)
    let allowTo = allow[p]
    if (allowTo === true) continue
    if (typeof allowTo !== 'object')
      throw new Error(`"allow"  refers to the unknown property ${allowTo})}`)
    for (let c in allowTo) {
      let role = allowTo[c]
      if (role === true) continue
      if (typeof role !== 'object')
        throw new Error(`"allow"  refers to the unknown property ${role})}`)
      for (let r in role) {
        if (!roles.includes(r))
          throw new Error(`"allow"  refers to the unknown property ${r})}`)
      }
    }
  }
}
