...
function checkRefsForModel ({ models, model }) {
  ...
  if (prerequisiteFor) {
    const prerequisiteForModel = models[prerequisiteFor]
    try {
      if (!prerequisiteForModel) {
        console.warn(`property "prerequisiteFor" of model ${model.id} ${referencesUnknownModel(model.id)}`)
      } else if (!isSubClassOf({ model: prerequisiteForModel, subModel: models[FINANCIAL_PRODUCT], models })) {
        throw new Error(`property "prerequisiteFor" should be subClassOf ${FINANCIAL_PRODUCT}`)
      } else if (!isSubClassOf({ model, subModel: models[MY_PRODUCT], models })) {
        console.warn(`property "prerequisiteFor" will be allowed only in subClassOf ${MY_PRODUCT}`)
      }
    } catch (err) {
